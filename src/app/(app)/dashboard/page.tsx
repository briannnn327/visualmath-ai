...
const statCards: Array<{
  label: string;
  icon: IconName;
  get: (d: Awaited<ReturnType<typeof getStudentData>>) => string | number;
  tone: string;
}> = [
  ...
  {
    label: "Soal Terpecahkan",
    icon: "check-circle",
    get: (d) => d.stats.solvedTotal,
    tone: "text-success",
  },
          ...
          <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex-1">
              ...
              <p className="font-display font-bold">
                Latih topik{" "}
                <span className="underline decoration-white/50">{data.recommendation.title}</span>
              </p>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Persentase penguasaan per topik berdasarkan latihan.
              </CardDescription>
            </div>
            <Badge variant="primary">
              {data.topics.filter((t) => t.progress.attempts > 0).length}/{data.topics.length}{" "}
              dimulai
            </Badge>
          </CardHeader>
          <CardContent className="space-y-4 pt-5">
            {data.topics.map(({ topic, progress }) => (
              <div
                key={topic.id}
                className="group rounded-2xl border border-outline-variant bg-surface p-4 transition-colors hover:border-primary/40"
              >
                <div className="mb-2 flex items-center gap-3">
                  ...
                  <Link href={`/latihan?topic=${topic.id}`}>
                    <Button
                      ...
                      size="sm"
                      icon="play"
                      aria-label={`Latihan ${topic.title}`}
                    />
                  </Link>
                </div>
                <Progress
            ...
            {[
              {
                ...
                text: "Mulai dari topik dengan penguasaan terendah lalu naik bertahap.",
              },
              {
                icon: "eye" as IconName,
                text: "Gunakan Grafik Interaktif untuk memahami bentuk fungsi sebelum menghitung.",
              },
              {
                icon: "target" as IconName,
                text: "Kerjakan 5 soal per sesi agar algoritma adaptif dapat menyesuaikan level.",
              },
              {
                icon: "flame" as IconName,
                text: "Pertahankan streak harian untuk XP dan rekomendasi yang lebih akurat.",
              },
            ].map((t) => (
              <div
                key={t.text}
                className="flex items-start gap-3 rounded-2xl border border-outline-variant bg-surface-container/50 p-3.5"
              >
